...
public class FilteredMessageHandler {
    ...
    public void handle(final FilterResult result, final Message message, final Subscription subscription) {
        if (result.filtered) {
            logger.debug("Message filtered [filter_type={}, subscription={}]",
                          result.filterType.get(), subscription.toSubscriptionName().toString());
            offsets.remove(message);
            updateMetrics(message, subscription);
            trackers.get(subscription).logFiltered(toMessageMetadata(message, subscription), result.filterType.get());
        }
    }
